...
<%
	...
	if((request.getParameter("act")).equals("Apply"))
	{
		...
		response.sendRedirect("../JobsDetails.jsp?msg=You application for job has been successfully submitted&job_id="+request.getParameter("job_id"));
	}
	if((request.getParameter("act")).equals("Save"))
	{
		...
		if((request.getParameter("job_id")).equals(""))
		{
			...
			response.sendRedirect("../report-job.jsp?customer_id="+session.getAttribute("customer_id")+"&msg=Job Saved Successfully");
		}
		else
		{
			results.put("job_id",request.getParameter("job_id"));
			out.println(jobObj.updateJob(results));
			response.sendRedirect("../report-job.jsp?customer_id="+session.getAttribute("customer_id")+"&msg=Job Updated Successfully");
		}
	}
%>

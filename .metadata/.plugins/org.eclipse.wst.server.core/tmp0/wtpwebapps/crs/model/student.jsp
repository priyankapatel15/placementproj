...
<%
	...
	if(request.getParameter("act").equals("login_status"))
	{
		if(session.getAttribute("student_id") == null) {
			response.sendRedirect("../student-login.jsp?msg=Login to continue !!!.");
		}
	}
	...
	if((request.getParameter("act")).equals("Save"))
	{
		...
		if((request.getParameter("student_id")).equals(""))
		{
			...
			response.sendRedirect("../student-login.jsp?msg=You have registered successfully. Login to your account !!!!");
		}
		else
		{
			results.put("student_id",request.getParameter("student_id"));
			...
			{
				response.sendRedirect("../student.jsp?student_id="+session.getAttribute("student_id")+"&msg=Account Updated Successfully");
			}
			else
			{
	...
	if((request.getParameter("act")).equals("chk_login"))
	{
		...
		{
			response.sendRedirect("../student-login.jsp?msg=Invalid User/Password. Please try again.........");			
		}
	}
	if((request.getParameter("act")).equals("logout"))
	{
		...
		response.sendRedirect("../student-login.jsp?msg=You are logout successfully......");			
	}
%>
